...
class PyQuake3(object):
    ...
    def send_packet(self, data):
        ...
        send packet
        """
        msg = self.packet_prefix + data.encode() + b"\n"
        self.sock.send(msg)
    ...
    def parse_status(self, data):
        ...
        """
        split = data[1:].split(b'\\')
        values = dict(list(zip(split[::2], split[1::2])))
        # if there are \n's in one of the values, it's the list of players
        for var, val in list(values.items()):
            pos = val.find(b'\n')
            if pos == -1:
                continue
            split = val.split(b'\n', 1)
...
if __name__ == '__main__':
    ...
    QUAKE = PyQuake3(server='roig.is-a.dev:27960', rcon_password='therconpassword')
